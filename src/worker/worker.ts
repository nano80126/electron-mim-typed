...
registerPromiseWorker(msg => {
	if (msg.type === 'csv') {
		...
		if (msg.furnace == 'vtech') {
			//
			const array: { datetime: number; data: number[] }[] = [];
			let names: string[] = []; // series names
			const txt = msg.data;
			const lines = txt.split('\n');

			let isValid = false; // 日期是否合理
			let dateType;
			let datetime = 0; // 紀錄 datetime, 避免xAxis 重疊

			for (let i = 0; i < lines.length; i++) {
				const items = lines[i].split(/\s{2,}|\t/); // 兩個以上空白字元
				...
				if (!isValid) {
					if (items[0] == ':DEV_COMMENT') {
						names = drop(items); // 去除 :DEV_COMMENT
						names = names.map(element => {
							return element.replace(/"/g, '');
						});
						continue;
			...
			return { type: 'txt', names, series: array };
		}
	}
});
