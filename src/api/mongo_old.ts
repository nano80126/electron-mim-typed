...
MongoClient.connect('mongodb://localhost:27017', { useUnifiedTopology: true }, (err, client) => {
	...
	const db = client.db('min-furnace');
	const config = db.collection('config');
	const hiper = db.collection('hiper');
	const vtech = db.collection('vtech');
	...
	ipcMain.handle('historySave', async (e, args) => {
		const ret = await history.updateOne(args.query, args.data, { upsert: true }); // upsert = insert + update
		return ret.result;
	});
